...
//}
    ...
    bool usedLogScale   = (NLoptDataStruct.ParameterLogScale ==  true);
    int m;
    int offset = 0;
    ...
    for (int i=0; i<NumSpeciesOrGuilds; ++i) {
        initBiomass.emplace_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
    }
    ...
    for (int i=0; i<NumSpeciesOrGuilds; ++i) {
        growthRate.push_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
    }
    offset += NumSpeciesOrGuilds;
    ...
    if (isLogistic) {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
            growthRateShape.emplace_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
//std::cout << "EstParameters[" << offset+i << "]: " << EstParameters[offset+i] << std::endl;
        }
        offset += NumSpeciesOrGuilds;
    } else {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
    ...
    for (int i=0; i<NumSpeciesOrGuilds; ++i) {
        growthRateCovariateCoeffs.emplace_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
    }
    ...
    if (isLogistic) {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
            carryingCapacity.emplace_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
        }
        offset += NumSpeciesOrGuilds;
    } else {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
    ...
    if (isLogistic) {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
            carryingCapacityCovariateCoeffs.emplace_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
        }
        offset += NumSpeciesOrGuilds;
    } else {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
    ...
    if (isCatchability) {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
            catchability.emplace_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
        }
        offset += NumSpeciesOrGuilds;
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
            catchabilityCovariateCoeffs.emplace_back(QString::number(EstParameters[offset+i],'f',nmfConstantsMSSPM::Output_Precision).toDouble());
        }
        offset += NumSpeciesOrGuilds;
    } else {
        for (int i=0; i<NumSpeciesOrGuilds; ++i) {
                                   ...
                                   void*         dataPtr)
{
    ...
    bool UseLogScaleForParameters = NLoptDataStruct.ParameterLogScale;
    bool isAggProd                = (NLoptDataStruct.CompetitionForm == "AGG-PROD");
...
//std::cout << "ObsBiomassBySpeciesOrGuilds(0,0): " << ObsBiomassBySpeciesOrGuilds(0,0) << std::endl;
...
//nmfUtils::printMatrix("carryingCapacityCovariate",carryingCapacityCovariate,16,10);

//double precision = 6; // RSK test code
// try rounding inside extractParameters...
...
//std::cout << "carryingCapacity[0]: " << carryingCapacity[0] << std::endl;
...
//EstBiomassTMinus1 = nmfUtils::round(EstBiomassTMinus1, precision);
...
//    if ((species == 5) && (time == 1)) {
//        std::cout << time << ": " << EstBiomassTMinus1 << ":: " <<
//                     growthRate[species] << ", " <<
//                     growthRateShape[species] << ", " <<
//                     growthRateCovariateCoeffs[species] << ", " <<
//                     growthRateCovariate(timeMinus1,species) << ", " <<
//                     carryingCapacity[species] << ", " <<
//                     carryingCapacityCovariateCoeffs[species] << ", " <<
//                     carryingCapacityCovariate(timeMinus1,species) << std::endl;
// //  if (time == NumYears-1) std::cout << std::endl;
//}
...
//GrowthTerm      = nmfUtils::round(GrowthTerm,     precision);
//HarvestTerm     = nmfUtils::round(HarvestTerm,    precision);
//CompetitionTerm = nmfUtils::round(CompetitionTerm,precision);
//PredationTerm   = nmfUtils::round(PredationTerm,  precision);

//if ((time >= 1) && (species == 5)) {
//  std::cout << "Time: [" << time << "] " << EstBiomassTMinus1 << ": "
//            << GrowthTerm << ", " << HarvestTerm
//            << ", " <<  CompetitionTerm << ", " << PredationTerm << std::endl;
//}

//if (species <3) {
//std::cout << "Terms: " << GrowthTerm << ", " << HarvestTerm << ", " <<  CompetitionTerm << ", " << PredationTerm << std::endl;
//  nmfUtils::printMatrix("predationRho",predationRho,16,10);
//  nmfUtils::printMatrix("predationRhoCovariate",predationRhoCovariate,16,10);
//}

//if (species == 1) {
//std::cout << "(" << time << "," << species << "): " << EstBiomassTMinus1 << "  ";
//}
//if ((species == 5) && (time == 1)) {
//  std::cout << EstBiomassTMinus1 <<  " += ";
//}
            EstBiomassTMinus1 +=  GrowthTerm - HarvestTerm - CompetitionTerm - PredationTerm;

//if ((species == 6) && (time == 25)) {
//  std::cout << GrowthTerm << " - " << HarvestTerm << " - " << CompetitionTerm << " - " << PredationTerm << std::endl;
//}
            EstBiomassTMinus1  = (EstBiomassTMinus1 < 0) ? NLoptDataStruct.MinimumBiomass[species] : EstBiomassTMinus1;
...
//nmfUtils::printMatrix("EstBiomassSpecies",EstBiomassSpecies,10,10);
...
//nmfUtils::printMatrix("ObsCatch",ObsCatch,10,10);
//nmfUtils::printMatrix("ObsCatchRescaled",ObsCatchRescaled,10,10);
//nmfUtils::printMatrix("ObsBiomassBySpeciesOrGuildsRescaled",ObsBiomassBySpeciesOrGuildsRescaled,10,10);
//nmfUtils::printMatrix("EstCatchRescaled",EstCatchRescaled,10,10);
//nmfUtils::printMatrix("EstBiomassRescaled",EstBiomassRescaled,14,10);
...
//nmfUtils::printMatrix("ObsBiomassTrimmed",ObsBiomassTrimmed,10,10);
//nmfUtils::printMatrix("EstCatchTrimmed",  EstCatchTrimmed,  10,10);
//nmfUtils::printMatrix("EstBiomassTrimmed",EstBiomassTrimmed,10,10);
...
//nmfUtils::printMatrix("ObsCatchRescaled",  ObsCatchRescaled,  16,10);
//nmfUtils::printMatrix("ObsBiomassBySpeciesOrGuildsRescaled",ObsBiomassBySpeciesOrGuildsRescaled,16,10);
//nmfUtils::printMatrix("EstCatchRescaled",  EstCatchRescaled,  16,10);
//nmfUtils::printMatrix("EstBiomassRescaled",EstBiomassRescaled,16,10);
...
NLopt_Estimator::setStoppingCriteria(nmfStructsQt::ModelDataStruct &NLoptStruct)
{
    ...
    if (NLoptStruct.NLoptUseStopVal) {
        ...
        // value by less than NLoptStruct.NLoptStopVal
                         ...
                         const bool& incrementFixedSeed)
{
    if (useUserFixedSeed && incrementFixedSeed) {
        nlopt::srand((unsigned long)m_FixedSeed++);
...
std::cout << "[Info 2] Using seed value: 1" << std::endl;
        ...
        if (isSetToDeterministic) {
            nlopt::srand(++m_Seed);
std::cout << "[Info 3] Using seed value: " << m_Seed << std::endl;
        } else {
            nlopt::srand_time();
                                    ...
                                    const int& NumEstParameters)
{
    ...
    // set seed for c++ rand() function
    if (m_isRangeJitterRepeatable) {
//double seedFactor = 1.0e6;
        srand(runNum);
//std::cout << "s1 - Setting c++ random seed to: " << runNum << std::endl;
    } else {
        unsigned seedVal = (unsigned)time(NULL)+runNum;
        srand(seedVal);
//std::cout << "s2 - Setting c++ random seed to: " << seedVal << std::endl;
    }
...
//qDebug() << "---> Num Parameter Bounds: " << NumEstParameters;
    ...
    emit UpdateNumberOfParameters(NumNonTrivialParameters);
}
                                    ...
                                    int& TotalIndividualRuns)
{
    ...
    checkAndApplyLogScale(NLoptStruct.ParameterLogScale, ParameterInitialValues, ParameterRanges);
...
//for (unsigned i=0; i<ParameterRanges.size(); ++i) {
//  std::cout << "ii: " << i << ": " << ParameterRanges[i].first << "," << ParameterRanges[i].second << std::endl;
//}
        ...
        std::string&       summaryMsg)
{
    ...
    std::cout << "Total Parameters: " << NLoptStruct.TotalNumberParameters << std::endl;
    std::cout << "Est'd Parameters: " << NLoptStruct.TotalNumberEstimatedParameters << std::endl;
    std::cout << "Est'd Parameters (non-trivial): " << NLoptStruct.TotalNumberEstimatedParametersNonTrivial << std::endl;
//  std::cout << "Fitness std dev: "  << fitnessStdDev << std::endl;
    ...
    summaryMsg += "<br>Est'd Parameters:&nbsp;"     + std::to_string(NLoptStruct.TotalNumberEstimatedParameters);
    summaryMsg += "<br>Est'd Parameters (non-trivial): " + std::to_string(NLoptStruct.TotalNumberEstimatedParametersNonTrivial);
    summaryMsg += "<br>";
    for (unsigned i=0; i<NLoptStruct.TotalNumberEstimatedParametersBySpecies.size(); ++i) {
        summaryMsg += "  " + std::to_string(NLoptStruct.TotalNumberEstimatedParametersBySpecies[i]);
    }

    summaryMsg += "<br><br><strong>Number of Runs:</strong>&nbsp;&nbsp;&nbsp;" + std::to_string(numSubRuns);
