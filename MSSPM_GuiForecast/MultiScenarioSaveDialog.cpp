...
void
MultiScenarioSaveDialog::callback_ShowSortPB()
{
...
void
MultiScenarioSaveDialog::loadWidgets()
{
    loadScenarioMap();
...
void
MultiScenarioSaveDialog::loadScenarioMap()
{
    m_databasePtr->createScenarioMap(m_orderedForecastLabelsMap);
}

std::string
MultiScenarioSaveDialog::getScenarioName()
{
    return ScenarioNameCMB->currentText().toStdString();
}

std::string
MultiScenarioSaveDialog::getForecastLabel()
{
    return ForecastLabelCMB->currentText().toStdString();
}

bool
MultiScenarioSaveDialog::getForecastData(
...
void
MultiScenarioSaveDialog::getOrderedForecastLabels(
...
void
MultiScenarioSaveDialog::callback_SetOrderPB()
{
    std::string cmd;
    std::string errorMsg;
    ...
    for (int sortOrder=0; sortOrder<ForecastLabelLW->count(); ++sortOrder) {
        ...
        cmd = "UPDATE ForecastBiomassMultiScenario SET SortOrder = " + std::to_string(sortOrder) +
...
void
MultiScenarioSaveDialog::callback_OkPB()
{
   ...
   if (okToWriteFile)
   {
       ...
       errorMsg = m_databasePtr->nmfUpdateDatabase(cmd);
       if (errorMsg != " ") {
           m_logger->logMsg(nmfConstants::Error,"[Error 1] MultiScenarioSaveDialog::callback_OkPB: DELETE error: " + errorMsg);
           m_logger->logMsg(nmfConstants::Error,"cmd: " + cmd);
           return;
       }

       cmd = "INSERT INTO ForecastBiomassMultiScenario (ScenarioName,SortOrder,ForecastLabel,SpeName,Year,Value) VALUES ";
       for (int i=0; i<NumSpecies; ++i) { // Species
           for (int j=0; j<NumYears; ++j) { // Time in years
               cmd += "('"   + Scenario +
                       "',"  + SortOrder +
       ...
       errorMsg = m_databasePtr->nmfUpdateDatabase(cmd);
       if (errorMsg != " ") {
           m_logger->logMsg(nmfConstants::Error,"[Error 2] MultiScenarioSaveDialog::callback_OkPB: Write table error: " + errorMsg);
           m_logger->logMsg(nmfConstants::Error,"cmd: " + cmd);
           return;
       }
       dataWritten = true;
...
void
MultiScenarioSaveDialog::callback_NewScenarioPB()
{
    bool ok;
...
void
MultiScenarioSaveDialog::callback_SetForecastPB()
{
    ForecastLabelCMB->setCurrentText(QString::fromStdString(m_forecastName));
}

void
MultiScenarioSaveDialog::callback_ScenarioNameCMB(QString scenario)
{
    int NumRecords;
    std::vector<std::string> fields;
    std::map<std::string, std::vector<std::string> > dataMap;
...
void
MultiScenarioSaveDialog::callback_DelScenarioPB()
{
    std::string scenario = getScenarioName();
    std::string cmd;
    std::string errorMsg;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        errorMsg = m_databasePtr->nmfUpdateDatabase(cmd);
        if (errorMsg != " ") {
            m_logger->logMsg(nmfConstants::Error,"[Error 1] MultiScenarioSaveDialog::callback_DelScenarioPB: DELETE error: " + errorMsg);
            m_logger->logMsg(nmfConstants::Error,"cmd: " + cmd);
            return;
        }
...
void
MultiScenarioSaveDialog::printMap(QString msg)
{
...
void
MultiScenarioSaveDialog::callback_DelForecastPB()
{
    ...
    if (okToDelete) {
        ...
        errorMsg = m_databasePtr->nmfUpdateDatabase(cmd);
        if (errorMsg != " ") {
            m_logger->logMsg(nmfConstants::Error,"[Error 1] MultiScenarioSaveDialog::callback_DelForecastPB: DELETE error: " + errorMsg);
            m_logger->logMsg(nmfConstants::Error,"cmd: " + cmd);
            return;
        }
...
void
MultiScenarioSaveDialog::callback_RenScenarioPB()
{
    bool ok;
...
void
MultiScenarioSaveDialog::callback_RenForecastPB()
{
    bool ok;
...
void
MultiScenarioSaveDialog::renameScenarioName(QString oldScenario,
                                            QString newScenario)
{
    std::string cmd;
    std::string errorMsg;
...
void
MultiScenarioSaveDialog::renameForecastLabel(QString scenario,
                                             QString oldForecast,
                                             QString newForecast)
{
    std::string cmd;
    ...
    cmd = "UPDATE ForecastBiomassMultiScenario SET ForecastLabel='" + newForecast.toStdString() +
